...
public class Karkkainen {
	private final static boolean leq(int a1, int a2, int b1, int b2) { // lexic. order for pairs
	...
	private final static void radixPass(int [] src, int [] dst, int [] v, int vi,
	     ...
	     */
	    static final int arrayMax(int [] input, int start, int length)
	    {
	        assert length >= 1;
	    ...
	    public static int [] buildSuffixArray(int [] input, int start, int length)
	    {
	        ...
	        final int alphabetSize = arrayMax(input, start, length);
	    ...
	    public static void main(String[] args) {
	    	char[] text = "una banana asesina".toCharArray();
	    	int[] intArray = new int[text.length];
	    	for(int i=0;i<text.length;i++) {
	    		intArray[i]=text[i];
	    	}
	    	int[] sa = buildSuffixArray(intArray, 0, intArray.length);
